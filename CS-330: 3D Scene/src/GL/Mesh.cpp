...
void Mesh::render(const std::shared_ptr<Shader>& shader) {
    ...
    for(int i=0; i < materials.size(); i++) {
        ...
        shader->setInt("useTexture" + std::to_string(i), 1);

        materials[i]->apply(i);
    }

    // if there are no materials on the mesh, we're not using textures
    if(materials.empty()) {
        shader->setInt("useTexture0", 0);
        shader->setInt("useTexture1", 0);
    }
    ...
    // turn the texture units off so they're not used on the next mesh
    for(int i=0; i < materials.size(); i++) {
        shader->setInt("useTexture" + std::to_string(i), 0);
        glActiveTexture(GL_TEXTURE0 + i);
        glBindTexture(GL_TEXTURE_2D, 0);
    }
}

void Mesh::setData(std::vector<Vertex> vertices, std::vector<GLushort> indices) {
    ...
    { // see note in render method
        ...
        glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(GLushort) * this->indices.size(), &this->indices[0], GL_STATIC_DRAW);
